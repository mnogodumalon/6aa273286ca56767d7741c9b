...
const listers: Record<EntityKey, () => Promise<RawRecord[]>> = {
  ...
  'berater/innen': () => LivingAppsService.getBeraterInnen() as Promise<RawRecord[]>,
...
const queriers: Record<EntityKey, (q: RecordQuery) => Promise<RawRecord[]>> = {
  ...
  'berater/innen': q => LivingAppsService.queryBeraterInnen(q) as Promise<RawRecord[]>,
...
const counters: Record<EntityKey, (filter?: string, signal?: AbortSignal) => Promise<number>> = {
  ...
  'berater/innen': (filter, signal) => LivingAppsService.countBeraterInnen(filter, signal),
...
const creators: Record<EntityKey, (fields: Record<string, unknown>) => Promise<RawMutation>> = {
  ...
  'berater/innen': fields => LivingAppsService.createBeraterInnenEntry(fields as never),
...
const updaters: Record<EntityKey, (id: string, fields: Record<string, unknown>) => Promise<RawMutation>> = {
  ...
  'berater/innen': (id, fields) => LivingAppsService.updateBeraterInnenEntry(id, fields as never),
