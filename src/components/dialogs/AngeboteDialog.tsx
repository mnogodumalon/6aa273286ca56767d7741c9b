...
export function AngeboteDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, projekteList, enablePhotoScan = true, enablePhotoLocation = true }: AngeboteDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="angebotstyp" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.angebotstyp) === 'dienstleistung'}
            onClick={() => setFields(f => ({ ...f, angebotstyp: (lookupKey(f.angebotstyp) === 'dienstleistung' ? undefined : 'dienstleistung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.angebotstyp) === 'dienstleistung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('angebote', 'angebotstyp', 'dienstleistung') ?? 'Dienstleistungsangebot'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.angebotstyp) === 'projekt'}
            onClick={() => setFields(f => ({ ...f, angebotstyp: (lookupKey(f.angebotstyp) === 'projekt' ? undefined : 'projekt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.angebotstyp) === 'projekt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('angebote', 'angebotstyp', 'projekt') ?? 'Projektangebot'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.angebotstyp) === 'wartung'}
            onClick={() => setFields(f => ({ ...f, angebotstyp: (lookupKey(f.angebotstyp) === 'wartung' ? undefined : 'wartung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.angebotstyp) === 'wartung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('angebote', 'angebotstyp', 'wartung') ?? 'Wartungsangebot'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.angebotstyp) === 'schulung'}
            onClick={() => setFields(f => ({ ...f, angebotstyp: (lookupKey(f.angebotstyp) === 'schulung' ? undefined : 'schulung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.angebotstyp) === 'schulung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('angebote', 'angebotstyp', 'schulung') ?? 'Schulungsangebot'}
          </button>
          <button
            type="button"
            role="radio"
    ...
    'angebotsdatum': (
      <div key="angebotsdatum" className="space-y-1.5">
      ...
      <div key="kostentyp" className="space-y-1.5">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="monatlich">{lookupLabel('angebote', 'kostentyp', 'monatlich') ?? 'Monatlich'}</SelectItem>
            <SelectItem value="jaehrlich">{lookupLabel('angebote', 'kostentyp', 'jaehrlich') ?? 'Jährlich'}</SelectItem>
            <SelectItem value="nach_aufwand">{lookupLabel('angebote', 'kostentyp', 'nach_aufwand') ?? 'Nach Aufwand'}</SelectItem>
            <SelectItem value="pauschal">{lookupLabel('angebote', 'kostentyp', 'pauschal') ?? 'Pauschal'}</SelectItem>
            <SelectItem value="sonstiges">{lookupLabel('angebote', 'kostentyp', 'sonstiges') ?? 'Sonstiges'}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    ),
    ...
    'kosten_beschreibung': (
      <div key="kosten_beschreibung" className="space-y-1.5">
    ...
    'angebotsbeschreibung': (
      <div key="angebotsbeschreibung" className="space-y-1.5">
    ...
    'vorlage_datei': (
      <div key="vorlage_datei" className="space-y-1.5">
        <Label htmlFor="vorlage_datei">{fieldLabel('angebote', 'vorlage_datei')}</Label>
