...
import type { Projekte, Kunden, BeraterInnen, LookupValue } from '@/types/app';
...
export function ProjekteDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, beraterInnenList, enablePhotoScan = true, enablePhotoLocation = true }: ProjekteDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="projektleitung" entity="Berater/innen">\n${JSON.stringify(beraterInnenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'akquise'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'akquise' ? undefined : 'akquise') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'akquise'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('projekte', 'status', 'akquise') ?? 'Akquise'}
          </button>
          <button
            type="button"
            role="radio"
