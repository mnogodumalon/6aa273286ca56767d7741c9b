...
import type { Zeiterfassung, BeraterInnen, Projekte, Leistungskatalog, LookupValue } from '@/types/app';
...
export type ZeiterfassungDialogDefaults = Omit<Zeiterfassung['fields'], 'monat'> & {
    monat?: LookupValue | string;
  };
...
export function ZeiterfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, beraterInnenList, projekteList, leistungskatalogList, enablePhotoScan = true, enablePhotoLocation = true }: ZeiterfassungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['datum', 'stunden', 'monat', 'jahr', 'berater', 'projekt'] as const;
  ...
  }), [beraterInnenList, projekteList, leistungskatalogList, ]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'monat': (
      <div key="monat" className="space-y-1.5">
        <Label htmlFor="monat">{fieldLabel('zeiterfassung', 'monat')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="august">{lookupLabel('zeiterfassung', 'monat', 'august') ?? 'August'}</SelectItem>
            <SelectItem value="september">{lookupLabel('zeiterfassung', 'monat', 'september') ?? 'September'}</SelectItem>
    ...
    'taetigkeitsbeschreibung': (
      <div key="taetigkeitsbeschreibung" className="space-y-1.5">
    ...
    'verrechenbar': (
      <div key="verrechenbar" className="space-y-1.5">
        <Label htmlFor="verrechenbar">{fieldLabel('zeiterfassung', 'verrechenbar')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="verrechenbar"
            checked={!!fields.verrechenbar}
            onCheckedChange={(v) => setFields(f => ({ ...f, verrechenbar: !!v }))}
          />
          <Label htmlFor="verrechenbar" className="font-normal">{fieldLabel('zeiterfassung', 'verrechenbar')}</Label>
        </div>
      </div>
    ),
    'notizen': (
  ...
  const FIELD_LABELS: Record<string, string> = {"datum": "Datum", "stunden": "Geleistete Stunden", "monat": "Abrechnungsmonat", "jahr": "Abrechnungsjahr", "taetigkeitsbeschreibung": "Tätigkeitsbeschreibung", "verrechenbar": "Verrechenbar", "notizen": "Notizen", "berater": "Berater/in", "projekt": "Projekt", "leistung": "Erbrachte Leistung"};
