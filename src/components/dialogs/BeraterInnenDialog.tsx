/**
 * BeraterInnenDialog — pre-generated create/edit dialog for BeraterInnen.
 *
 ...
 * BeraterInnenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<BeraterInnen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<BeraterInnenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { BeraterInnen, Leistungskatalog, Projekte, LookupValue } from '@/types/app';
...
/** Widened prefill type for BeraterInnenDialog.defaultValues — see file header. */
export type BeraterInnenDialogDefaults = Omit<BeraterInnen['fields'], 'status'> & {
    status?: LookupValue | string;
  };

interface BeraterInnenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: BeraterInnen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: BeraterInnenDialogDefaults;
...
export function BeraterInnenDialog({ open, onClose, onSubmit, defaultValues, recordId, leistungskatalogList, projekteList, enablePhotoScan = true, enablePhotoLocation = true }: BeraterInnenDialogProps) {
  const [fields, setFields] = useState<Partial<BeraterInnen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<BeraterInnen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'berater/innen');
      await onSubmit(clean as BeraterInnen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["leistungen", "projekte"]);
        ...
        if (Array.isArray(leistungenNames) && leistungenNames.length > 0) {
          const leistungenUrls = (leistungenNames as unknown[])
            .map(n => leistungskatalogList.find(r => matchName(String(n), [String(r.fields.leistungsbezeichnung ?? '')])))
        ...
        const projekteNames = raw['projekte'];
        if (Array.isArray(projekteNames) && projekteNames.length > 0) {
          const projekteUrls = (projekteNames as unknown[])
          ...
          if (projekteUrls.length > 0) merged['projekte'] = projekteUrls;
        }
        return merged as Partial<BeraterInnen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('berater/innen') })
    : t('new_entity', { entity: appLabel('berater/innen') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'email_beruflich': (
      <div key="email_beruflich" className="space-y-1.5">
        <Label htmlFor="email_beruflich">{fieldLabel('berater/innen', 'email_beruflich')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email_beruflich"
          type="email"
          inputMode="email"
    ...
    'email_privat': (
      <div key="email_privat" className="space-y-1.5">
        <Label htmlFor="email_privat">{fieldLabel('berater/innen', 'email_privat')}</Label>
        <Input
          id="email_privat"
          type="email"
          inputMode="email"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('berater/innen', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('berater/innen', 'status', 'urlaub') ?? 'Urlaub'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('berater/innen', 'status', 'elternzeit') ?? 'Elternzeit'}
          </button>
          <button
            type="button"
            role="radio"
    ...
    'stundensatz': (
      <div key="stundensatz" className="space-y-1.5">
        <Label htmlFor="stundensatz">{fieldLabel('berater/innen', 'stundensatz')}</Label>
        <Input
          id="stundensatz"
          type="number"
          inputMode="decimal"
    ...
    'sonstiges_1': (
      <div key="sonstiges_1" className="space-y-1.5">
    ...
    'sonstiges_2': (
      <div key="sonstiges_2" className="space-y-1.5">
    ...
    'projekte': (
      <div key="projekte" className="space-y-1.5">
        ...
        <MultiCombobox
          ...
          values={extractRecordIds(fields.projekte)}
          onChange={ids => setFields(f => ({ ...f, projekte: ids.length ? ids.map(id => createRecordUrl(APP_IDS.PROJEKTE, id)) as any : undefined }))}
          onCreateNew={(q) => openCreateProjekte("projekte", q)}
