...
export function LeistungskatalogDialog({ open, onClose, onSubmit, defaultValues, recordId, beraterInnenList, enablePhotoScan = true, enablePhotoLocation = true }: LeistungskatalogDialogProps) {
  ...
  const REQUIRED_FIELDS = ['leistungsbezeichnung', 'leistungstyp'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'stundensatz_leistung': (
      <div key="stundensatz_leistung" className="space-y-1.5">
        <Label htmlFor="stundensatz_leistung">{fieldLabel('leistungskatalog', 'stundensatz_leistung')}</Label>
        <Input
    ...
    'einheit': (
      <div key="einheit" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('leistungskatalog', 'einheit', 'stunde') ?? 'Stunde'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('leistungskatalog', 'einheit', 'tag') ?? 'Tag'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.einheit) === 'monat'}
            onClick={() => setFields(f => ({ ...f, einheit: (lookupKey(f.einheit) === 'monat' ? undefined : 'monat') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.einheit) === 'monat'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('leistungskatalog', 'einheit', 'monat') ?? 'Monat'}
          </button>
        </div>
      </div>
    ),
